...
class KMeansPlusPlus extends Program with ProgramDescription with Serializable {
	...
	def getScalaPlan(dop: Int, dataPointInput: String, clusterOutput: String, n:Int, k: Int, numIterations: Int) = {
		...
		// compute the distance between each point and all current centroids
			...
			val nextCenter = nearestCenter reduceAll {generateNextCenter}

			centers union {nextCenter} map { center => center}
 ...
 */
object RunKMeansPlusPlus {

	def main(args: Array[String]) {
		val km = new KMeansPlusPlus
		if (args.size < 6) {
			println(km.getDescription)
			return
		}
		val plan = km.getScalaPlan(args(0).toInt, args(1), args(2), args(3).toInt, args(4).toInt, args(5).toInt)
