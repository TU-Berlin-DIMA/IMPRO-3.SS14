...
public class KMeansppDouble1D {
	...
	public static final class StringPointConverter extends MapFunction<String, MyPoint> {
		...
		public MyPoint map(String value) throws Exception {
			...
			return new MyPoint(Double.parseDouble(tmp[0]), Double.parseDouble(tmp[1]));
		}
	}

	public static class MyFunctions implements GenericFunctions1D<MyPoint> {
	...
	public static void main(String[] args) throws Exception {
		...
		KMeansppGeneric1D<MyPoint> kmp = new KMeansppGeneric1D<MyPoint>(dop, args[2], k, itrs, epsilon);
		kmp.run(MyPoint.class, new MyFunctions(args[1]));
	}
