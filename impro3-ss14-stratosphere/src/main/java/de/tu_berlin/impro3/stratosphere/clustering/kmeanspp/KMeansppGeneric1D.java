...
public class KMeansppGeneric1D<T> implements Serializable {

	private static final long serialVersionUID = 3203582521287201233L;

	public KMeansppGeneric1D(int dop, String outputPath, int numClusters, int maxIterations, double terminateEpsilon) {
		this.dop = dop;
		this.outputPath = outputPath;
		this.numClusters = numClusters;
		this.numIterations = maxIterations;
		this.EPSILON = terminateEpsilon;
	}

	public void run(Class<T> typeClass, GenericFunctions1D<T> function) throws Exception {
		...
		DataSet<Tuple2<Integer, T>> kmeansppTmpCenters = distance.reduceGroup(new PickWithDistance())
			.withBroadcastSet(sum.cross(addition), "sumcrosscentroid")
			.union(addition)
			.map(new DummyMap());

		DataSet<Tuple2<Integer, T>> kmeansInitCenters = addition.closeWith(kmeansppTmpCenters);
		kmeansInitCenters.writeAsCsv(new Path(outputPath, "initcenters").toString(), "\n", "|");
		
		// ========================== standard K-Means procedure ==================================

		// set number of bulk iterations for KMeans algorithm
		IterativeDataSet<Tuple2<Integer, T>> iteration = kmeansInitCenters.iterate(numIterations);
		...
		DataSet<Tuple2<Integer, T>> finalCentroids = iteration.closeWith(
				...
				kmeansTmpCenters.join(iteration).where(0).equalTo(0).filter(new EpsilonFilter(function)));
	...
	public final class DummyMap extends MapFunction<Tuple2<Integer, T>, Tuple2<Integer, T>> {

		private static final long serialVersionUID = -8266144860463722495L;

		@Override
		public Tuple2<Integer, T> map(Tuple2<Integer, T> p) throws Exception {
			return p;
		}
	}
	...
	public final class SelectNearestCenter extends MapFunction<T, Tuple2<Integer, T>> {
		...
		private GenericFunctions1D<T> function;
		private Collection<Tuple2<Integer, T>> centroids;

		public SelectNearestCenter(GenericFunctions1D<T> function) {
	...
	public final class SelectNearestDistance extends MapFunction<T, Tuple2<T, Double>> {
		...
		public SelectNearestDistance(GenericFunctions1D<T> function) {
			this.function = function;
		}
	...
	public final class CentroidAccumulator extends ReduceFunction<Tuple3<Integer, T, Long>> {
		...
		public CentroidAccumulator(GenericFunctions1D<T> function) {
			this.function = function;
		}
		@Override
	...
	public final class CentroidAverager extends MapFunction<Tuple3<Integer, T, Long>, Tuple2<Integer, T>> {
		...
		public CentroidAverager(GenericFunctions1D<T> function) {
			this.function = function;
		}
		@Override
	...
	public final class EpsilonFilter extends FilterFunction<Tuple2<Tuple2<Integer,T>,Tuple2<Integer,T>>> {
		
		/**
		 * 
		 */
		private static final long serialVersionUID = -2106701714193572964L;
		private GenericFunctions1D<T> function;
		
		public EpsilonFilter(GenericFunctions1D<T> function) {
			this.function = function;
		}

		@Override
		public boolean filter(Tuple2<Tuple2<Integer, T>, Tuple2<Integer, T>> value)
				throws Exception {
			System.out.println(String.format("Filter %d,%s with %d,%s", value.f0.f0, value.f0.f1.toString(), value.f1.f0, value.f1.f1.toString()));

			return function.distance(value.f0.f1, value.f1.f1) > EPSILON;
		}
	}
