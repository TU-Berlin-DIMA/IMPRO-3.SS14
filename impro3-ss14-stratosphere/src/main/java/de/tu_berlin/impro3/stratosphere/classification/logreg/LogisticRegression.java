...
public class LogisticRegression {
	...
	/**
	 * This class represents a labeled point in space.
	 * It consists of a feature vector (represented as array of doubles) and an Integer label.
	 */
	 ...
	 * @throws Exception
	 */
	public static void runProgram(final int numberOfFeatures, String pointsWithLabelsPath, float alpha, int maxIterations, String outputPathTheta, int labelPosition) throws Exception {
		...
		DataSet<Integer> numberOfPoints = pointsWithLabels
				.map(new MapFunction<Point, Integer>() {
					@Override
					public Integer map(Point t)
							throws Exception {
						return 1;
					}			
				})
		...
		DataSet<Gradient> sumGradient = gradient
				.reduce(new ReduceFunction<Gradient>() {
					
					/**
					 * Builds the sum to obtain full average gradient
					 */
					@Override
					public Gradient reduce(Gradient gradient1, Gradient gradient2)
							throws Exception {
						// grad(i) +=
		...
		DataSet<Theta> modifiedTheta = sumGradient.crossWithTiny(iteration).with(new CrossFunction<LogisticRegression.Gradient, LogisticRegression.Theta, Theta>() {
			private Float alpha;
	...
	/**
	 * This class is used to parse an input text file into an DataSet of Points.
	 * Label position specifies the position of the label value in the CSV file
	 * 
	 * @return DataSet of Points
	 */
	public static DataSet<Point> parsePointsWithLabels(ExecutionEnvironment env, String pointsWithLabelsPath, final int numberOfFeatures, final int labelPosition) {
		return env
			.readTextFile(pointsWithLabelsPath)
